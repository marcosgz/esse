...
RSpec.describe Esse::Index, 'RequestConfigurable' do
  ...
  describe ".combined_request_body_for?" do
    let(:index) { Class.new(Esse::Index) }

    context "when both index and cluster have request body for the operation" do
      it "returns false" do
        expect(index.combined_request_body_for?(:update)).to be false
      end
    end

    context "when index has request body for the operation" do
      before do
        index.request_body(:update, refresh: true)
      end

      it "returns true" do
        expect(index.combined_request_body_for?(:update)).to be true
      end
    end

    context "when cluster has request body for the operation" do
      before do
        index.cluster.request_body(:update, refresh: true)
      end
  ...
  describe ".combined_request_body_for" do
    ...
    context "when both index and cluster have request body for the operation" do
      before do
        index.request_body(:update, refresh: true)
        index.cluster.request_body(:update, timeout: '5s', refresh: false)
      end

      it "returns merged request body" do
        expect(index.combined_request_body_for(:update, doc)).to eq(refresh: true, timeout: '5s')
      end
    end
    ...
    context "when index has request body for the operation" do
      ...
      it "returns index request body" do
        expect(index.combined_request_body_for(:update, doc)).to eq(refresh: true)
      end
    end

    context "when cluster has request body for the operation" do
      before do
        index.cluster.request_body(:update, timeout: '5s')
      end

      it "returns cluster request body" do
        expect(index.combined_request_body_for(:update, doc)).to eq(timeout: '5s')
      end
    end

    context "when neither index nor cluster have request body for the operation" do
      it "returns an empty hash" do
        expect(index.combined_request_body_for(:update, doc)).to eq({})
      end
    end
  end
end
