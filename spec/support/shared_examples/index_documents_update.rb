...
RSpec.shared_examples 'index.update' do
  ...
  it 'raises an Esse::Transport::ServerError exception when api throws an error' do
    es_client do |client, _conf, cluster|
      expect {
        VenuesIndex.update(id: -1, body: { doc: { name: 'foo' } })
  ...
  it 'raises ArgumentError when the :id is not provided' do
    es_client do |client, _conf, cluster|
      expect {
        VenuesIndex.update(body: { doc: { name: 'foo' } })
  ...
  it 'updates the document in the aliased index' do
    es_client do |client, _conf, cluster|
      ...
      resp = nil
      expect {
        resp = VenuesIndex.update(id: 1, body: { doc: { name: 'New Name' } })
      }.not_to raise_error
  ...
  it 'updates the document in the unaliased index' do
    es_client do |client, _conf, cluster|
      ...
      resp = nil
      expect {
        resp = VenuesIndex.update(id: 1, body: { doc: { name: 'New Name' } }, suffix: index_suffix)
      }.not_to raise_error
      expect(resp['result']).to eq('updated')
  ...
  it 'updates the document using the instance of Esse::Serializer' do
    es_client do |client, _conf, cluster|
      ...
      resp = nil
      expect {
        resp = VenuesIndex.update(Esse::HashDocument.new(id: 1, name: 'New Name'))
      }.not_to raise_error
      expect(resp['result']).to eq('updated')
