...
RSpec.describe Esse::CLI::Index, type: :cli do
  describe '#delete' do
    ...
    context 'with a valid index name' do
      ...
      specify do
        expect(CountiesIndex).to receive(:delete_index).and_return(true)
        cli_exec(%w[index delete CountiesIndex])
      end

      specify do
        expect(CountiesIndex).to receive(:delete_index).with(suffix: 'foo').and_return(true)
        cli_exec(%w[index delete CountiesIndex --suffix=foo])
      end

      it 'allows multiple indices' do
        expect(CountiesIndex).to receive(:delete_index).and_return(true)
        expect(CitiesIndex).to receive(:delete_index).and_return(true)
