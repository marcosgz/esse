...
RSpec.describe Esse::CLI::Index, type: :cli do
  describe '#open' do
    ...
    context 'with a valid index name' do
      ...
      specify do
        expect(CountiesIndex).to receive(:open).and_return(true)
        cli_exec(%w[index open CountiesIndex])
      end

      specify do
        expect(CountiesIndex).to receive(:open).with(suffix: 'foo').and_return(true)
        cli_exec(%w[index open CountiesIndex --suffix=foo])
      end

      it 'allows multiple indices' do
        expect(CountiesIndex).to receive(:open).and_return(true)
        expect(CitiesIndex).to receive(:open).and_return(true)
