...
RSpec.shared_examples 'cluster_api#count' do
  it 'checks the number of documents in an index' do
    es_client do |_client, _conf, cluster|
      ...
      cluster.api.index(index: index_name, id: 1, body: { name: 'Illinois', pk: 1 }, refresh: true)

      resp = nil
      expect {
        resp = cluster.api.count(index: index_name)
      }.not_to raise_error
  ...
  it 'raises an error when the index does not exist' do
    es_client do |_client, _conf, cluster|
      ...
      expect {
        cluster.api.count(index: index_name)
