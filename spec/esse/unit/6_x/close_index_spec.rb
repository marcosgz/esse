...
RSpec.describe "[ES #{ENV.fetch("STACK_VERSION", "6.x")}] elasticsearch#index", es_version: '6.x', es_webmock: true do
  before do
    stub_index(:geos) do
      define_type :city
      define_type :county
  ...
  describe '.close!', events: %w[elasticsearch.close] do
    it 'raises an exception if the elasticsearch-api throws an error' do
      body = elasticsearch_response_fixture(file: 'index_not_found', version: '6.x', assigns: { index_name: 'geos' })
