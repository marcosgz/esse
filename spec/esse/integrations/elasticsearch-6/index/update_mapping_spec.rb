...
stack_describe 'elasticsearch', '6.x', Esse::Index, '.update_mapping' do
  ...
  it 'raises an Esse::Transport::ServerError exception when api throws an error' do
    es_client do |client, _conf, cluster|
      expect {
        GeosIndex.update_mapping(suffix: index_suffix, type: '_doc')
  ...
  it 'update mappings from index definition' do
    es_client do |client, _conf, cluster|
      ...
      resp = nil
      expect {
        resp = GeosIndex.update_mapping(suffix: index_suffix, type: '_doc')
      }.not_to raise_error
      ...
      expect(mapping.dig(index_name, 'mappings', '_doc', 'properties', 'new_field')).to eq(
        'type' => 'text',
      )
    end
  end
  ...
  it 'update mappings from body argument' do
    es_client do |client, _conf, cluster|
      ...
      resp = nil
      expect {
        resp = GeosIndex.update_mapping(suffix: index_suffix, type: '_doc', body: {
          _doc: {
            properties: {
              state_abbr: { type: 'keyword' },
              new_field: { type: 'text' },
