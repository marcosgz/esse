...
RSpec.describe Esse::Backend::Index do
  ...
  describe '.open!' do
    specify do
      es_client do |client, _conf, cluster|
        expect{ DummiesIndex.backend.open! }.to raise_error(
    ...
    specify do
      es_client do |client, _conf, cluster|
        expect{ DummiesIndex.backend.open!(suffix: 'v1') }.to raise_error(
          Elasticsearch::Transport::Transport::Errors::NotFound,
  ...
  describe '.open' do
    specify do
      es_client do |client, _conf, cluster|
        expect(DummiesIndex.backend.open).to eq(false)
      end
    end

    specify do
      es_client do |client, _conf, cluster|
        expect(DummiesIndex.backend.open(suffix: 'v1')).to eq(false)
      end
    end
