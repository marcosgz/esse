...
RSpec.shared_examples 'cluster_api#update' do
  it 'updates an existing document' do
    es_client do |_client, _conf, cluster|
      ...
      resp = nil
      expect {
        resp = cluster.api.update(index: index_name, id: 1, body: { doc: { name: 'IL' } })
      }.not_to raise_error
  ...
  it 'raises an error when document does not exist' do
    es_client do |_client, _conf, cluster|
      ...
      expect {
        cluster.api.update(index: index_name, id: 1, body: { doc: { name: 'IL' } })
